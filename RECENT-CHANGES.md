...
### 30. Autopilot: фикс §3.4 — бизнес-метки ОБЯЗАНЫ ставиться автопилотом при create (2026-05-18)

**Контекст.** Пилотный прогон на `novosibirsk-ai` обнажил противоречие в правилах меток. В рамках волны #29 (state-based ownership) в `.claude/skills/leadgen-autopilot/skill.md` §3.4 попала фраза:

> «Бизнес-метки Этажей — НЕ трогаем. Их ставит человек по правилам labels.md. Автопилот их только читает для mapping…»

Эта строка **противоречила** остальной экосистеме скилла:
- `.claude/skills/leadgen/config/labels.md` HARD RULE: «автопилот ставит **только** бизнес-метки».
- `lessons_registry.md` lesson `−1`: запрещены **internal**-метки, но не бизнес.
- `branches/{apply,onboarding}.md` + `references/action_catalog.md` — все говорят про `add_labels` с бизнес-метками.
- `leadgen/branches/create-search.md` Шаг 9 + `create-rsya.md` Шаг R9 — обязательный `add_labels` стандартного набора.

В прогоне `novosibirsk-ai-2026-05-18-0832` автопилот, опираясь на §3.4, **пропустил Шаг 9** при создании DRAFT search-кампании zagorodka 709972678 — кампания получилась без бизнес-меток (пустой набор), статистика бы не попала в каталог Этажей.

**Фикс.**

- `skill.md` §3.4 переписан: явно сказано, что автопилот **ОБЯЗАН** поставить бизнес-метки при `campaign.create_draft.*` через `add_labels` по таблице `labels.md` (Лидген + тематика + направление + канал-метка + спец-теги). На **adopted/foreign**-кампаниях бот их **не модифицирует** (read-only — это решение оператора). Internal-метки (`autopilot:*`, `city:*`, `topic:*`, `channel:*`) по-прежнему **не ставит никогда**.
- В рамках прогона `0905` сделан backfill бизнес-меток на live-кампаниях, созданных под этим багом:
  - `709972678` Поиск Загородка: добавлен полный набор `Лидген, Загородка, Покупатель, Поиск` (tags 41990699-41990702) на 10 ads.
  - `709914995` Поиск Вторичка: добавлена недостающая метка `Поиск` (tag 41990703) на 18 ads.
  - `709915110` РСЯ Вторичка: уже имела полный набор — без изменений.
- В прогоне `0905` запущена RSYA Загородка 709973230 (DRAFT) — впервые применена корректная версия §3.4: 15 ads сразу получили `Лидген, Загородка, Покупатель, РСЯ` через `add_labels`.
- `autopilot/learnings/proposed/direct-api-suspend-before-archive-2026-05.md` — гипотеза из прогона `0736`: Direct API отвергает `manage_ads(action=archive)` на showing-ads (code 8300), нужен двухшаговый suspend→archive. Ждёт ещё 2 повторений для валидации.

**Эффект.** Любая будущая campaign.create_draft.* автоматически получит правильный набор бизнес-меток → статистика будет агрегироваться в каталоге Этажей с первого дня. Конфликт между skill.md и labels.md устранён — единственный источник правды по бизнес-меткам теперь `leadgen/config/labels.md`.
