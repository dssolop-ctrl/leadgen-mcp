...
| Инструмент | Описание |
|---|---|
| `check_search_volume` | Популярные запросы с точной частотностью + ассоциации (до 128 фраз). Параметры: `phrases` (через запятую), `region_ids` (опционально) |
...
1. **Быстрая проверка частотности** — вызвать MCP `check_search_volume(phrases="фраза1,фраза2")` для списка базовых фраз. Отсечь фразы с нулевой частотностью.

2. **Расширение семантики** — вызвать MCP `check_search_volume(phrases="<базовая фраза>")` для каждой базовой фразы. Собрать все связанные запросы и ассоциации.
