...
**Бизнес-метки Этажей (`Лидген`, `Вторичка`/`Загородка`/`Новостройки`/..., `Покупатель`/`Продавец`, `Поиск`/`РСЯ`/`Мастер`/..., специальные типа `Имидж`/`HR`) — автопилот ОБЯЗАН поставить при `campaign.create_draft.*`** через `add_labels` по таблице в `leadgen/config/labels.md`. Без бизнес-меток статистика кампании не попадёт в каталог/отчёты Этажей. На **существующих adopted/foreign**-кампаниях автопилот бизнес-метки **не модифицирует** (read-only) — это решение оператора.

Internal-метки (`autopilot:*`, `city:*`, `topic:*`, `channel:*`) автопилот **не ставит** никогда (см. выше). Каталог `leadgen/config/labels.md` — единственный источник правды по бизнес-меткам.
