...
2. **Разрешение и aspect** — ≥ 450×450 для 1:1, ≥ 1080×607 для 16:9.

   ⚠ **Yandex Direct API применяет проверку aspect строже заявленного ±2%.** Эмпирически (тест 2026-05-05): 1376×768 (отклонение 0.8% от 16:9) → ошибка 5004 «Размер изображения некорректен». Ratio должен быть **точным** или в пределах ±0.5%.

   **Решение:** серверный auto-crop в `generate_image` / `generate_banner_set` (с 2026-05-05) теперь даёт точные размеры:
   - 1:1 → ровный квадрат (например 768×768 при crop из 1024×768)
   - 16:9 → 1024×576 (ratio = 1.7778, точно 16/9)
   - 4:3 → 1024×768 (ratio = 1.3333)
   - 3:2 → 1620×1080 (ratio = 1.5)

   После auto-crop **НЕ нужно** дополнительно ресайзить локально через PIL. Передавай `file_path` из ответа в `add_ad_image` напрямую.
